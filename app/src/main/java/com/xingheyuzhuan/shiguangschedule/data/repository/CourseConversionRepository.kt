...
class CourseConversionRepository @Inject constructor(
    ...
    suspend fun importCourseTableFromJson(
        ...
        // 日志：追踪实际写入量
        Log.d("CourseImport", "importCourseTableFromJson: 准备写入 courseEntities=${courseEntities.size}, " +
                "courseWeekEntities=${courseWeekEntities.size}")
        val coursesWithNullSection = courseEntities.filter { it.startSection == null || it.endSection == null }
        if (coursesWithNullSection.isNotEmpty()) {
            Log.w("CourseImport", "  其中 ${coursesWithNullSection.size} 个课程的 startSection/endSection 为 null，" +
                    "这些课程在 mergeCourses 中会被过滤掉不显示！")
            coursesWithNullSection.forEach {
                Log.w("CourseImport", "    name=${it.name}, day=${it.day}, " +
